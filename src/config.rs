...
pub struct Config {
    ...
    /// Load and save the config to the config file, by default is true.
    #[serde(skip, default = "default_persistent")]
    persistent: bool,
    /// Used to store an original config, when we enter regime with `!persistent`
    #[serde(skip, default = "default_stored")]
    stored: Option<Box<Config>>,
}
...
fn default_stored() -> Option<Box<Config>> {
    None
}
...
impl Config {
    ...
    pub fn load_or_create(persistent: bool) -> Result<Self> {
        if persistent
            || match std::env::var("VSL_CLI_PERSISTENT_CONFIG") {
                Ok(val) => val != "0",
                Err(_) => false,
            }
        {
            let config_path = vsl_config_dir()?.join("cli.json");
            ...
            match File::open(&config_path) {
                ...
                Err(e) if e.kind() == io::ErrorKind::NotFound => {
                    // File doesn't exist, create a new default config
                    let config = Config::new(true);
            ...
            // In case `VSL_CLI_PERSISTENT_CONFIG` is set to 0, use a default clear config, do not load saved.
            Ok(Config::new(false))
        }
    }

    pub fn save(&self) -> Result<()> {
        // In case `VSL_CLI_PERSISTENT_CONFIG` is set to 0, the config is not saved.
        if self.persistent {
            let config_path = vsl_config_dir()?.join("cli.json");
    ...
    /// Store the current state of a config in `stored`
    pub fn store_state(&mut self) -> Result<()> {
        if self.stored.is_some() {
            return Err(anyhow::anyhow!("Already have a stored config state"));
        }
        self.stored = Some(Box::new(self.clone()));
        // Do not save temporary config state
        self.persistent = false;
        Ok(())
    }
    /// Restore the original state of a config from `stored`
    pub fn restore_state(&mut self) -> Result<()> {
        match &self.stored {
            ...
            None => Err(anyhow::anyhow!("No stored config state is present")),
        }
    }
}
